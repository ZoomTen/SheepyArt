...
def delete_from_collection(collection, art, rm_collection_name=True):
    ...
    if collection:
        if art:
            if (current_user.id == collection.user_id):
                entry = CollectionData.query.filter_by(art_id=art.id,collection_id=collection.id).first()
                try:
                    db.session.delete(entry)
                    db.session.commit()
                except:
                    Response('There was an error with deleting from the collection', 500)
                # FIXME: collections: make better responses
                if rm_collection_name:
                    return Response(f"{art.title} successfully removed from {collection.title}", 200)
                else:
                    return Response(f'Successfully removed from favorites!', 200)
            return Response("You don't own this collection", 403)
        return Response('No art specified!', 403)
    return Response('No collection specified!', 403)


@collection.route('/add_to_collection', methods=['POST'])
@login_required
...
@collection.route('/remove_from_collection', methods=['POST'])
@login_required
def user_rm_collection():
    collection = None
    art = None
    collection_id = request.form.get('collection')
    if collection_id:
    ...
    return delete_from_collection(collection, art)


@collection.route('/remove_favorite', methods=['POST'])
@login_required
def user_rm_favorite():
    # Get user's favorites table
    collection = CollectionMeta.query.filter_by(use_as_favorites=1, user_id=current_user.id).first()
    art = None
    art_id = request.form.get('art')
    ...
    return delete_from_collection(collection, art, rm_collection_name=False)
